...
void hw_Init (void)
{
    ...
    MCG->C2 = MCG_C2_RANGE(0x02) | MCG_C2_EREFS_MASK; /* Very-high freq range + enable crystal oscillator (25 MHz XTAL on EXTAL0/XTAL0) */
    ...
    while((MCG->S & MCG_S_OSCINIT0_MASK) == 0x00U); /* Wait until crystal oscillator has started up */
    while((MCG->S & MCG_S_IREFST_MASK) != 0x00U); /* Check external reference validation */
    MCG->C5 = MCG_C5_PRDIV0(0x07); /* PLL ref divider /8 -> 25 MHz / 8 = 3.125 MHz */
